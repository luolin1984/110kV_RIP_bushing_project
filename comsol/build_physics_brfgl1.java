...
public class build_physics_brfgl1 {
  ...
  private static void setCaseParameters(Model model) {
    ...
    model.param().set("h_air_case", "5[W/(m^2*K)] + 4[W/(m^2*K)]*(wind_case/(1[m/s]))^0.8");
    model.param().set("h_oil_case", "100[W/(m^2*K)]");
  ...
  private static void addMaterials(Model model) {
    ...
    material(model, "mat_rip", "RIP capacitor core", "v2_rip_capacitor_core", "0.10[W/(m*K)]", "2210[kg/m^3]", "730[J/(kg*K)]", "epsr_rip", "sigma_rip");
    ...
    material(model, "mat_air", "Surrounding air", "v2_surrounding_air_domain", "0.026[W/(m*K)]", "1.2[kg/m^3]", "1005[J/(kg*K)]", "1", "1e-15[S/m]");
    material(model, "mat_oil", "Transformer oil", "v2_surrounding_oil_domain", "0.12[W/(m*K)]", "850[kg/m^3]", "1900[J/(kg*K)]", "2.2", "1e-12[S/m]");
  ...
  private static void addHeatTransfer(Model model) {
    ...
    heatSource(model, "hs_cu", "v2_center_conductor", "Icase^2*(1/sigma_cu_20)*(1+alpha_cu*(T-293.15[K]))/(pi*(r_conductor_outer^2-r_hollow^2))^2");
    heatSource(model, "hs_contact", "v2_contact_resistance_heat_source_layer", "Q_contact_vol");
    heatSource(model, "hs_rip_dielectric", "v2_rip_capacitor_core", "omega0*epsilon0_const*epsr_rip*tan_delta_rip*es.normE^2");
    convectiveFlux(model, "hf_air", "v2_bnd_air_external_convection", "h_air_case", "Tair_case");
    convectiveFlux(model, "hf_oil", "v2_bnd_oil_immersed_surface", "h_oil_case", "Toil_case");
  }
  ...
  private static void addOperators(Model model) {
    ...
    maxop(model, "maxop_T_conductor", "geom_v2", "v2_center_conductor");
    maxop(model, "maxop_T_rip", "geom_v2", "v2_rip_capacitor_core");
    maxop(model, "maxop_T_contact", "geom_v2", "v2_contact_resistance_heat_source_layer");
    maxop(model, "maxop_E_all", "geom_v2", "all");
    maxop(model, "maxop_E_rip", "geom_v2", "v2_rip_capacitor_core");
  ...
  private static void addMesh(Model model) {
    ...
    model.component("comp_v2").mesh("mesh1").autoMeshSize(5);
  }
