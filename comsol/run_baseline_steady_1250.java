...
public class run_baseline_steady_1250 {
  ...
  public static void main(String[] args) throws java.io.IOException {
    ...
    String note = "RUN002 CAD-driven baseline";
    try {
    ...
    model.save(new File(root, "comsol/BRFGL1-126-1250-4_baseline_STEADY_1250_LOAD_1p0_RUN002.mph").getPath());
  }

  private static void export(Model model, String root, String status, String note) throws IOException {
    File outDir = new File(root, "results/raw_comsol_exports/STEADY_1250_LOAD_1p0_RUN002");
    outDir.mkdirs();
    ...
    if (!Double.isNaN(tmaxAll) && (tmaxAll < -60.0 || tmaxAll > 250.0)) {
      status = "INVALID_TEMPERATURE_RANGE";
      note = note + "; Tmax_global_C outside physical acceptance range [-60, 250] C";
    }
    ...
    try {
      ...
      writer.write("STEADY_1250_LOAD_1p0,RUN002," + status + ","
    ...
    if (!"SOLVED".equals(status)) {
      ...
      try {
        reportWriter.write("# STEADY_1250_LOAD_1p0 RUN002 exception report\n\n");
