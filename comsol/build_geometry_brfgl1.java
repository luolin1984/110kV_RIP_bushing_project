...
public class build_geometry_brfgl1 {
  ...
  private static void createSelections(Model model, String comp, String geom) throws IOException {
    ...
    if (comp.equals("comp_v2")) {
      ...
      unionSelection(model, comp, p + "bnd_air_external_convection_explicit", p + "bnd_air_convection_seg_", airCount);
      unionSelection(model, comp, p + "bnd_oil_immersed_surface_explicit", p + "bnd_oil_convection_seg_", oilCount);
    }
  ...
  private static int addCadBoundarySelections(Model model, String comp, String geom, String csvPath,
    ...
    while ((line = reader.readLine()) != null) {
      ...
      boxSel(model, comp, geom, selectionPrefix + threeDigit(idx), 1,
          (rOuter - 0.05) + "[mm]",
          (rOuter + 0.05) + "[mm]",
          (z0 + 0.02) + "[mm]",
          (z1 - 0.02) + "[mm]");
  ...
  private static void unionSelection(Model model, String comp, String unionTag, String selectionPrefix, int count) {
    try {
      model.component(comp).selection().create(unionTag, "Union");
      model.component(comp).selection(unionTag).geom(comp.equals("comp_v2") ? "geom_v2" : "geom_v1", 1);
  ...
  private static void writeConvectionBoundaryCsvs() throws IOException {
    writeConvectionBoundaryCsv("air", "v2_bnd_air_convection_seg_",
        ...
        0.0, 1150.0,
        "CAD air-side silicone shed external surface");
    writeConvectionBoundaryCsv("oil", "v2_bnd_oil_convection_seg_",
        ...
        -595.0, 0.0,
        "CAD oil-side immersed core/shield external surface");
  }

  private static void writeConvectionBoundaryCsv(String prefix, String selectionPrefix, String inputCsv,
      String outputCsv, double minZ, double maxZ, String physicalMeaning) throws IOException {
