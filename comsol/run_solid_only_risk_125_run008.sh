...
LOG_DIR="$PROJECT_DIR/results/raw_comsol_exports/SOLID_ONLY_RISK_125_RUN008"
...
"$COMSOL_BIN" compile "$PROJECT_DIR/comsol/run_solid_only_risk_125_run008.java" 2>&1 | tee "$LOG_DIR/compile_risk_125.log"
"$COMSOL_BIN" batch \
  -autosave off \
  -inputfile "$PROJECT_DIR/comsol/run_solid_only_risk_125_run008.class" \
  -batchlog "$LOG_DIR/risk_125_batch.log" \
  -batchlogout 2>&1 | tee "$LOG_DIR/risk_125_stdout.log"
