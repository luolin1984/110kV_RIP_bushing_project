...
The baseline full electro-thermal calculation uses STEADY_1250_LOAD_1p0 with 1250 A current, 72.75 kV phase-to-ground RMS voltage, 85 °C oil temperature, 25 °C air temperature, and R_c_factor = 1. RUN009A is solved as valid and gives a global maximum temperature of 88.589 °C. The conductor, RIP, contact layer, silicone housing, and flange maxima are 74.136 °C, 88.589 °C, 58.790 °C, 57.215 °C, and 46.906 °C, respectively. The region-specific values are not identical, which confirms that the post-processing selections preserve physical region identity. The baseline potential, electric-field, dielectric-loss, and temperature indicators are shown in Fig. 2.
...
RUN010 extends the field-coupled formulation to 125 steady operating cases. Across the matrix, Tmax_global ranges from 67.397 °C to 134.974 °C, while Tmax_contact reaches a maximum of 129.925 °C. Fig. 3 presents the global temperature response, and Fig. 4 presents the corresponding contact-region response.
...
These risk zones are internal diagnostic visualization classes. They are not IEC/IEEE standard limits, material lifetime thresholds, or operating guarantees. A diagnostic-risk-method reference should be added to support the general concept of using risk classes for numerical screening while preserving this distinction [REF_RISK_DIAGNOSTIC_METHOD].

## 4.4 Contribution of field-coupled dielectric loss

RUN008 used an approximate dielectric-loss reference in the solid-only diagnostic model, whereas RUN010 uses the field-coupled loss integrated from the electrostatic solution. Replacing the reference loss with the field-coupled term increases Tmax_global by an average of 0.617 °C and a maximum of 2.816 °C. The risk zone changes in 2 of the 125 cases, as shown in Fig. 7.
...
For these reasons, RUN010 should be treated as a audited finite-element numerical screening scan, not as a final operational rule. The next manuscript-development step should add externally verifiable references for geometry, standards, material properties, electrostatics, dielectric-loss theory, mesh convergence, and diagnostic-risk terminology before moving into the Introduction and literature review.
...
2. The RUN009A baseline gives 88.589 °C global maximum temperature and 28.049 W field-coupled RIP dielectric loss, with a field/reference ratio of 11.020. The DIELECTRIC_LOSS_REVIEW_REQUIRED flag is retained and interpreted as a review item rather than a failed run.
3. In the RUN010 125-case matrix, Tmax_global_C ranges from 67.397-134.974 °C and Tmax_contact_C reaches 129.925 °C.
...
5. Replacing the approximate dielectric-loss reference with field-coupled dielectric loss changes Tmax_global by 0.617 °C mean delta_Tmax and 2.816 °C max delta_Tmax, with 2/125 changed risk zones.
