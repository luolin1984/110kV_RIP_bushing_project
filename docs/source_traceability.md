...
| RUN002_THERMAL_CALIBRATION | RUN002热量级修正与基准热场校准 | `results/raw_comsol_exports/STEADY_1250_LOAD_1p0_RUN002/baseline_metrics.csv` | COMSOL基准运行与RIP导热率临时校准 | 否 | 是 | 否 | 否 | 将RIP热导率暂取`0.20 W/(m*K)`以获得物理合理的RUN002基准温度；必须在论文使用前做材料来源替换或敏感性分析。 |
